...
https://bioinfohsl-tools.s3.amazonaws.com/goliath/$OUTNAME/abundance.h5
https://bioinfohsl-tools.s3.amazonaws.com/goliath/$OUTNAME/run_info.json
https://bioinfohsl-tools.s3.amazonaws.com/goliath/$OUTNAME/abundance.tsv
EOF
...
cpaws() {
	...
	echo ":: Copy the output to AWS" >&2
        aws s3 cp  "$OUTNAME" s3://bioinfohsl-tools/goliath/"$OUTNAME" --acl public-read --expires "$EXP_DATE" --recursive
}
